...
namespace dev.susybaka.Shared.Audio
{
    public class AudioManager : MonoBehaviour
    {
        ...
        private void SetupSoundSource(Sound s, AudioSource source, int i = 0)
        {
            ...
            source.outputAudioMixerGroup = s.mixerGroup != null ? s.mixerGroup : templateSource.outputAudioMixerGroup;
        ...
        public void StopPlaying(AudioMixerGroup mixerGroup)
        {
            if (log)
                Debug.Log("[AudioManager] Stopping playback of all sounds in mixer group: " + (mixerGroup != null ? mixerGroup.name : "null"));

            foreach (Sound s in sounds)
            {
                // Check if the sound's mixer group matches the provided one
                if (s.mixerGroup == mixerGroup)
                {
                    if (!s.hasMultipleSources && s.source != null)
                    {
